...
Level = (function() {
  ...
  Level.prototype.topBlockAt = function(x) {
    var row, xGrid, y, _i, _len, _ref;
    xGrid = Math.floor((x + this.progress) / gfx.tileW + 1);
    _ref = this.map;
    for (y = _i = 0, _len = _ref.length; _i < _len; y = ++_i) {
      row = _ref[y];
...
Player = (function() {
  ...
  Player.prototype.move = function(xo, yo) {
    this.updateSurface();
    if ((this.y >= this.surface - this.h) && !this.onGround) {
      this.falling = false;
      this.onGround = true;
      this.land();
    }
  ...
  Player.prototype.updateSurface = function() {
    return this.surface = this.level.topBlockAt(this.x);
  };
